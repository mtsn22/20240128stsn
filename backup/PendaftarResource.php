    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                SelectColumn::make('ps_kelas')
                    ->label('Kelas')
                    ->options(Kelas::query()->pluck('kelas', 'id')),

                SelectColumn::make('ps_tahap')
                    ->options([
                        'Tahap 1' => 'Tahap 1',
                        'Tahap 2' => 'Tahap 2',
                        'Diterima' => 'Diterima',
                    ...
                    ->beforeStateUpdated(function ($record, $state) {
                        // dd($record);
                        if ($state === 'Tahap 1') {
                            if ($record->kelas_calon !== null) {

                                if (User::where('username', $record->kk_calon)->count() === 0) {

                                    $user = new user;

                                    $user->name = $record->ak_nama_lengkap_calon;
                                    $user->username = $record->kk_calon;
                                    $user->password = Str::camel($record->kk_calon);
                                    $user->panelrole = 'walisantri';
                                ...
                                Notification::make()
                                    ->title('Harap mengisi kelas calon santri!')
                                    ->warning()
                                    ->color('warning')
                                    ->icon('heroicon-o-exclamation-circle')
                                    ->iconColor('warning')
                                    ->persistent()
                                    // ->actions([Action::make('kelas_calon')
                                    //     ->button([Action::make('kelas_calon')
                                    //         ->form([
                                    //             Select::make('kelas_calon')
                                    //                 ->label('Kelas')
                                    //                 ->options(Kelas::query()->pluck('kelas', 'id'))
                                    //                 ->required(),
                                    //         ])
                                    //         ->action(function (array $data, Pendaftar $record): void {
                                    //             $record->kelas_calon()->associate($data['kelas_calon']);
                                    //             $record->save();
                                    //         })])])
                                    ->send();
